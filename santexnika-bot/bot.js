...
// Restart bir marta schedullanishi uchun flag
let restartScheduled = false;

function scheduleRestart(delayMs) {
  if (restartScheduled) return;
  restartScheduled = true;
  console.log(`Restart ${delayMs / 1000}s dan keyin...`);
  setTimeout(() => {
...
bot.on("polling_error", (err) => {
  const code = err.code || "";
  console.error("Polling xatolik:", code, err.message?.slice(0, 80));
...
// ─── Mahsulotlar ro'yxati (split by category — 4096 limit) ───────────────────
...
async function generateExcel(chatId) {
  ...
  const catColors = [
    "FFFFF8E1", "FFE3F2FD", "FFF3E5F5", "FFE8F5E9",
    "FFFCE4EC", "FFFFE8D6", "FFE0F2F1", "FFF9FBE7",
  ];
...
bot.on("message", async (msg) => {
  ...
  if (state?.step === "waiting_category_name") {
    ...
    if (data.categories.some((c) => c.toLowerCase() === catName.toLowerCase())) {
      return bot.sendMessage(chatId, `⚠️ *"${catName}"* allaqachon mavjud.`, { parse_mode: "Markdown" });
    }
...
bot.on("callback_query", async (query) => {
  ...
  if (cbData.startsWith("cat_view:")) {
    ...
    for (let i = 0; i < prods.length; i++) {
      const line = `${i + 1}. ${prods[i].name}\n`;
      if ((text + line).length > LIMIT) {
