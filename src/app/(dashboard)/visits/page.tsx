...
export default function VisitsPage() {
  ...
  const handleSearchMembers = async () => {
    ...
    setSearching(true);
    try {
      const data = await api.getMembers({ search: memberSearch.trim(), limit: 10 });
      ...
      <Modal isOpen={scanModalOpen} onClose={() => { setScanModalOpen(false); setMemberSearch(""); setSearchResults([]); }}
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={handleSearchMembers} disabled={searching}
              ...
              {searching ? <Loader2 size={16} className="animate-spin" /> : "Qidirish"}
            </button>
          </div>
